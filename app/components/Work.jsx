...
function Work() {
  return (
    ...
    id='work' className='w-full px-[12%] py-10 scroll-m-28'>
        ...
        className='text-center text-5xl font-mono'>Projetos</motion.h2>
        <motion.p 
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        transition={{ duration: 0.5, delay: 0.7 }}  
        className='text-center max-w-2xl mx-auto mt-5 mb-12 font-mono'>Bem-vindo ao meu portfólio. Explore uma coleção de projetos que demonstram minha expertise.</motion.p>
        ...
        {workData.map((project, index)=>(
            ...
            style={{backgroundImage: `url(${project.bgImage})`}}>
                <div className='bg-[#FCFCF2] w-10/12 rounded-md absolute bottom-5 left-1/2 -translate-x-1/2 py-3 px-5 flex items-center justify-between duration-500 group-hover:bottom-7'>
                    <div>
                        <h2 className='font-semibold font-mono'>{project.title}</h2>
                        <p className='text-sm text-gray-700 font-mono'>{project.description}</p>
                    </div>
    ...
    href="" className='w-max flex items-center justify-center gap-2 text-gray-700 border-[0.5px]
     border-gray-700 rounded-full py-3 px-10 mx-auto my-20 hover:bg-lightHover duration-500 font-mono'>
